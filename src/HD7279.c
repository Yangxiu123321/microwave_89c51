...
unsigned char xdata realCode[] = \
	{0x7e , 0x30 , 0x6d , 0x79 , 0x33 , 0x5b , 0x5f , 0x70 , 0x7f , 0x7b , 0x01 , 0x80 , 0x00 , 0x6f , 0x48};//15
...
void LEDShowInt(long int showData)
{
	...
	if(showData<=99999999&&showData>=-9999999)
	{
		...
		for(i = 0; i< 8; i++)
		{
			// 经过sprintf后没有写的位置为NULL,ascill值为0
			if(tempBuff[i]!=0x00)
			{
				dataLength++;
			}
...
* @brief	显示字符串
  * @note
* @param tempBuff:要显示的字符串
  *     @arg
  * @param
  * @retval
  */
void LEDShowString(char* tempBuff)
{
	//要发送的8个数据
	unsigned char xdata LedData[8] = {0x00};
	//循环技术变量
	...
	if(tempBuff != NULL)
	{
		...
		for(i = dataLength;i > 0;i--)
		{
			if(tempBuff[i-1]!='-'&&tempBuff[i-1]!='.'&&tempBuff[i-1]!='=')
			{
				LedData[7 - (dataLength - i)]=tempBuff[i-1] - 0x30;
				LedData[7 - (dataLength - i)] = realCode[LedData[7 - (dataLength - i)]];
			}
			...
			{
				LedData[7 - (dataLength - i)] = realCode[14];	
			}
		}
...
/**
  ...
  * @retval
  */
void LEDShowSec(long int showData)
{
	//要发送的八个数据
	unsigned char xdata LedData[8] = {0x00};
	//循环计数变量
...
* @param tempBuff:要显示的字符串
  ...
  * @retval
  */
void ShowYourNeed(void)
{
	unsigned char xdata i = 0;
	unsigned char decoder[]={0};
	for(i = 0;i<8;i++)
	{
		LedWrite(0x97 - i , decoder[i]);
	}
}